...
export class Init1680372808490 implements MigrationInterface {
    name = 'Init1680372808490'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`workloads\` (\`id\` int NOT NULL AUTO_INCREMENT, \`value\` varchar(255) NOT NULL, \`user_id\` int NOT NULL, \`project_id\` int NOT NULL, \`start_date\` datetime NOT NULL, \`end_date\` datetime NOT NULL, \`userId\` int NULL, \`projectId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`users_kanbans\` ADD CONSTRAINT \`FK_c52087f52f5c4bf5401fa466ffa\` FOREIGN KEY (\`kanban_id\`) REFERENCES \`kanbans\`(\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
        await queryRunner.query(`ALTER TABLE \`users_kanbans\` ADD CONSTRAINT \`FK_add019dc84b4fa2c84569964d96\` FOREIGN KEY (\`user_id\`) REFERENCES \`users\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
